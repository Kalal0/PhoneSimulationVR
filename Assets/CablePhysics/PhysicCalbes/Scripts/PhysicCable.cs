...
namespace HPhysic
{
    public class PhysicCable : MonoBehaviour
    {
        ...
        [Header("CustomAdditions")]
        public Transform MovingPoint; // The other point to measure the distance from
        public XRDirectInteractor handInteractor;
        public float maxDistance; // Maximum allowed distance
        public Transform KinematicPoint;
        public XRWhenAmIBeingGrabbed grabcheckerscript;
        public HandleSataConnectionONOFF handleConOnOff;
        ...
        private void Update()
        {
            ...
            if (handleConOnOff.Connector1IsConnected == true || handleConOnOff.Connector2IsConnected == true || handleConOnOff.Connector3IsConnected == true)
            {
                // Calculate current distance
                float currentDistance = Vector3.Distance(KinematicPoint.position, MovingPoint.position);
                ...
                if (currentDistance > maxDistance)
                {
                    if (handInteractor != null && handInteractor.isSelectActive)
                    {
                        ...
                        var selectedObject = handInteractor.interactablesSelected;

                        if (selectedObject != null && selectedObject.Count != 0)
                        {
                            // Force the hand to release the object
                            handInteractor.interactionManager.SelectExit(handInteractor, selectedObject[0]);
